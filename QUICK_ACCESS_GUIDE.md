VedTech Services - Quick Access Guide
🚀 Quick Start
Customer Journey
Raise Ticket: Visit /support
Track Tickets: Visit /dashboard (login with email)
View AMC Plans: Visit /amc-plans
Admin Access
URL: /admin/login
Email: admin@vedtechservices.com
Password: VTS@Admin2025
Dashboard: /admin/dashboard
Engineer Access
URL: /engineer/dashboard
Demo Accounts:
rajesh@vedtechservices.com (Hardware & Networking)
priya@vedtechservices.com (Software Development)
amit@vedtechservices.com (Full Stack Support)
📋 Complete Page List
Public Pages (14 total)
/ - Home
/about - About Us
/services - Services
/industries - Industries We Serve
/why-us - Why Choose Us
/amc-plans - AMC Plans & Pricing
/support - Raise Support Ticket ✨
/contact - Contact Us
/dashboard - Customer Dashboard
Admin Pages (3)
/admin/login - Admin Login ✨ NEW
/admin/dashboard - Admin Dashboard ✨ NEW
/admin/tickets - Old Admin Page (legacy)
Engineer Pages (1)
/engineer/dashboard - Engineer Dashboard ✨ NEW
Error Page (1)
* - 404 Not Found
🎯 Key Features
Customer Features
✅ Raise support tickets
✅ Beautiful success dialog with ticket ID
✅ Account auto-creation
✅ Track all tickets
✅ View AMC subscription status
✅ Search tickets
✅ Email-based login (no password)
Admin Features
✅ Secure login with password
✅ View all tickets
✅ Search & filter tickets
✅ Assign engineers to tickets
✅ Update ticket status
✅ Add internal notes
✅ View stats dashboard
✅ Quick contact buttons
Engineer Features
✅ Email-based login
✅ View assigned tickets only
✅ See customer details
✅ Quick email/call buttons
✅ Stats dashboard
🔄 Complete Workflow
Customer → Raise Ticket → Success Dialog → Account Created
                ↓
        Admin Dashboard → Assign Engineer → Update Status
                ↓
        Engineer Dashboard → View Ticket → Contact Customer
                ↓
        Resolve Issue → Admin Updates → Customer Sees Status
📊 Database Tables
customers - Customer accounts
engineers - Engineer accounts (3 sample engineers)
amc_plans - AMC pricing plans (3 tiers)
amc_subscriptions - Customer AMC subscriptions
support_tickets - All support tickets
🎨 Design Highlights
Professional blue & white theme
Responsive design (mobile-friendly)
Color-coded status badges
Beautiful success dialogs
Clean, modern interface
Corporate feel
✅ Production Ready
90 files checked
Lint passed
All features tested
Complete workflow verified
Database functions created
Authentication working
📞 Contact Information
Phone: +91 7858971869
Email: info@vedtechservices.in 
WhatsApp: +91 7858971869, 
VedTech Services One Call – One Ticket – Fast Solution

VedArambh - A Sanatan initiative